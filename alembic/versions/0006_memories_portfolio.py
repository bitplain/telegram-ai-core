"""memories and portfolio_assets tables; remove openrouter_api_key from app_settings
...
def upgrade() -> None:
    op.execute(sa.text("DELETE FROM app_settings WHERE key = 'openrouter_api_key'"))
    ...
    op.create_table(
        ...
        sa.Column("average_buy_price", sa.Numeric(38, 8), nullable=False),
    ...
    op.create_index(
        "ix_portfolio_assets_user_symbol_network",
        "portfolio_assets",
...
def downgrade() -> None:
    op.drop_index("ix_portfolio_assets_user_symbol_network", table_name="portfolio_assets")
