...
- Ручной **портфель ETH** (`/portfolio`, `/add_eth`), опциональные **новости** (RSS + опционально CryptoPanic), **контекст для crypto-агента** (баланс из БД, цена CoinGecko, заголовки с URL), **алерты по цене ETH** и **ежедневный digest** (UTC), без вызова LLM для `/portfolio`.
...
- `app/core/price/`, `app/core/news/` — внешние источники (CoinGecko, RSS, опционально CryptoPanic).
- `app/bot/background_workers.py` — фоновые циклы алертов и digest (только при заданном `TELEGRAM_BOT_TOKEN`).
...
Без `TELEGRAM_BOT_TOKEN` polling и фоновые воркеры (алерты, digest) не стартуют, FastAPI работает.
Без `OPENROUTER_API_KEY` бот ответит «OpenRouter API key не настроен…» на обычные LLM-сообщения; команды `/portfolio`, `/add_eth`, алерты и digest при этом работают там, где не нужен LLM (digest без ключа уходит без блока «Кратко (LLM)»).
...
### Портфель, алерты, digest, внешние данные

- **`/portfolio`** — показывает ручной баланс ETH из БД, при доступности — цену ETH (CoinGecko) и оценку в USD; **LLM не вызывается**. Это **ручной учёт в боте**, не реальный кошелёк и не on-chain баланс.
- **`/add_eth <amount>`** — добавить положительное количество к учётному балансу (хранится как `Numeric`/`Decimal` в PostgreSQL). Уменьшение баланса отдельной командой пока не реализовано.
- **`/alert_eth <usd>`** — одноразовый алерт: цель в USD, направление `above` / `below` относительно **текущей** цены CoinGecko; если цена недоступна или цель равна текущей — алерт не создаётся. **Алерты зависят от CoinGecko.**
- **`/alerts`** — список активных алертов пользователя.
- **`/alert_cancel all`** — деактивировать все активные алерты (без нумерации по id).
- **`/digest_on`**, **`/digest_off`**, **`/digest_status`** — ежедневный digest в личку; **время отправки в UTC** (`DAILY_DIGEST_HOUR_UTC`). При пустом `OPENROUTER_API_KEY` digest уходит **без** LLM-блока «Кратко». Новости берутся из RSS (CoinDesk, The Block, Cointelegraph) и опционально CryptoPanic — источники могут быть **недоступны**, тогда в тексте явный fallback (ничего не выдумывается).

Опциональные переменные (см. `.env.example`):

- `CRYPTOPANIC_API_KEY` — **необязательно**; при пустом значении CryptoPanic не вызывается, приложение стартует нормально.
- `ETH_ALERT_CHECK_INTERVAL_SECONDS` — период опроса алертов (секунды).
- `DAILY_DIGEST_ENABLED`, `DAILY_DIGEST_HOUR_UTC`, `DAILY_DIGEST_POLL_INTERVAL_SECONDS` — digest и его расписание (UTC).

Фоновые воркеры алертов и digest запускаются **только если задан** `TELEGRAM_BOT_TOKEN`. Эндпоинт **`/health`** по-прежнему не ходит во внешние API; **`/ready`** проверяет только PostgreSQL и Redis.
