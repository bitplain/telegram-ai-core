...
- aiogram-бот: `TELEGRAM_MODE=polling` (long-polling) или `TELEGRAM_MODE=webhook` (FastAPI вызывает `setWebhook` при старте, см. ниже).
...
Без `OPENROUTER_API_KEY` бот ответит «OpenRouter API key не настроен. Добавь OPENROUTER_API_KEY в переменные окружения.». Ключ **только из ENV**, в БД не сохраняется.

### Webhook (`TELEGRAM_MODE=webhook`)

- Задай `PUBLIC_API_URL` (например `https://your-service.up.railway.app`) без trailing slash и путь из `TELEGRAM_WEBHOOK_PATH` (по умолчанию `/telegram/webhook`).
- Задай `TELEGRAM_WEBHOOK_SECRET` — тот же токен передаётся в `setWebhook(secret_token=...)`; Telegram шлёт его в заголовке `X-Telegram-Bot-Api-Secret-Token`. Неверный заголовок → **403**.
- Если режим не `webhook`, `POST /telegram/webhook` отвечает **410** (не «успешный» 200).
- В `polling` webhook в Telegram лучше снять: при shutdown вызывается `delete_webhook`.

### Память и портфель (MVP)

- `/remember текст` — глобальная память (видна всем агентам в LLM-контексте).
- `/remember_agent текст` — память только для текущего `active_agent_id`.
- `/memory` — список global + agent memories.
- `/forget_memory` или `/forget_memory <uuid>` — удаление по ID.
- `/portfolio` — активы ETH (ручной учёт), сумма ETH и средневзвешенная цена; watch-only кошельки.
- `/portfolio_add_eth amount price network` — докупка с пересчётом средней (например `/portfolio_add_eth 0.25 3200 arbitrum`).
- `/portfolio_wallet_add name network address` — только адрес (watch-only). Сообщения с признаками private key / seed / mnemonic отклоняются: «Нельзя сохранять приватные ключи или seed-фразы.»
- `/portfolio_wallets` — список сохранённых адресов.
...
4. В сервисе приложения открой **Variables** и добавь:
   - `APP_ENV=railway`
   - `TELEGRAM_MODE=polling` (или `webhook` + `PUBLIC_API_URL` + `TELEGRAM_WEBHOOK_SECRET`)
...
- **OpenRouter API key** — только переменная окружения `OPENROUTER_API_KEY`.
  Через бота ключ **не** вводится и **не** хранится в `app_settings`.
...
- Нет pgvector / RAG; краткая память через таблицу `memories` и команды `/remember*`, `/memory`, `/forget_memory`; история чата ограничена `max_context_messages`.
- Нет managed bots в смысле Bot API 9.5 mini-app-flow — только обычный полноценный бот.
- Webhook поддерживается (`TELEGRAM_MODE=webhook`, `PUBLIC_API_URL`, `TELEGRAM_WEBHOOK_SECRET`); для локальных проверок удобен polling.
