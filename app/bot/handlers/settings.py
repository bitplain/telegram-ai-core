...
def _openrouter_keyboard() -> InlineKeyboardMarkup:
    rows: list[list[InlineKeyboardButton]] = [
        [InlineKeyboardButton(text="Избранное", callback_data=SettingsCB(action="favorites").pack())],
        [InlineKeyboardButton(text="Обновить список моделей", callback_data=SettingsCB(action="refresh").pack())],
...
async def _settings_status_text() -> str:
    ...
    env_key = (get_settings().OPENROUTER_API_KEY or "").strip()
    openrouter_src = "ENV" if env_key else "не задан"
    openrouter_line = f"OpenRouter: <b>{openrouter_src}</b>" + (
        f" ({_mask_key(env_key)})" if env_key else ""
    )
...
async def _render_openrouter_menu(message: Message) -> None:
    ...
    body = ["<b>OpenRouter</b>"]
    if env_key:
        body.append(f"API-ключ: <b>ENV</b> ({_mask_key(env_key)})")
        body.append("Ключ задаётся только через переменную <code>OPENROUTER_API_KEY</code>.")
    else:
        body.append("API-ключ: <b>не задан</b> (задайте <code>OPENROUTER_API_KEY</code> в окружении).")
    body.append("Избранные модели и overrides ниже.")
    await _edit_or_answer(message, "\n\n".join(body), _openrouter_keyboard())
...
async def cb_refresh(callback: CallbackQuery, callback_data: SettingsCB, state: FSMContext) -> None:
    ...
    if isinstance(callback.message, Message):
        await _edit_or_answer(callback.message, text, _openrouter_keyboard())
    await callback.answer("Готово")
