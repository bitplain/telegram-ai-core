...
class UserAgentSettingsService:
    ...
    async def get_effective_settings(
        ...
        if row and row.model_id:
            selected_model = self._model_registry.get(row.model_id)
    ...
    async def set_model_id(
        ...
        agent = self._require_agent(agent_id)
        model = self._model_registry.get_or_none(model_id)
        if model is None or not model.enabled:
            raise UnknownModelError(model_id)
        return await self._with_repository(
            lambda repo: repo.upsert_model_id(telegram_user_id, agent.id, model.id)
        )
